...
def compute_elo_rankings(data):
    ...
    for i in range(1,len(data)):
        ...
        if i%5000==0:
            ...
            print(f'{i} of {len(data)} matches computed...')
    print(f'Done! {i} of {len(data)} matches computed...')
    ranking_elo=pd.DataFrame(ranking_elo,columns=["elo_winner","elo_loser"])    
    ranking_elo["proba_elo"]=1 / (1 + 10 ** ((ranking_elo["elo_loser"] - ranking_elo["elo_winner"]) / 400)) 
    ranking_elo["proba_elo_loser"]=1 / (1 + 10 ** ((ranking_elo["elo_winner"] - ranking_elo["elo_loser"]) / 400)) 
    ranking_elo["elo_diff"]=(ranking_elo["elo_winner"] - ranking_elo["elo_loser"])
    return ranking_elo
